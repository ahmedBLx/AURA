...
const OffersPage = () => {
    ...
    // Filter enabled subcategories for Offers (memoized)
    const offersSubCategories = useMemo(() => {
        return categories.filter(
            (c) => c.parentName === 'Offers' && c.showOnHomepage === true
        );
    }, [categories]);

    // Filter products dynamically for Offers (memoized)
    ...
    const mainFilteredProducts = useMemo(() => {
        return getFilteredSubProducts(offersProducts);
    }, [offersProducts, getFilteredSubProducts]);
