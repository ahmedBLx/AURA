...
const OffersPage = ({ onOpenAuth }) => {
    ...
    // Map products to their offer details dynamically
    const getOfferDetails = (p) => {
    ...
    // Filter products that have a valid discount percent set or Offers category
    const offerProducts = products.filter(p => (p.categories && p.categories.includes('Offers')) || (p.discountPercent && p.discountPercent > 0));
        ...
        <main style={{ minHeight: '85vh', padding: '60px 80px', position: 'relative' }}>
            ...
            <div className={`product-grid ${layout === 'list' ? 'list-layout' : ''}`} id="offers-product-grid">
                {filteredProducts.length === 0 ? (
                    ...
                    const { salePrice, discountText } = getOfferDetails(p);
                    return (
                        <div className="product-card item-card" key={p.id} data-id={p.id}>
                            <div className="card-image-box">
                                <span className="badge badge-special" style={{ backgroundColor: '#EF4444', color: '#FFFFFF', border: '1px solid #EF4444' }}>{discountText}</span>
                                <img src={p.img} alt={p.name} className="product-img" />
                                {layout !== 'list' && (
                                    <div className="card-overlay-actions">
                                        <button className="quick-view-btn" onClick={() => handleQuickView(p, salePrice)}>Quick View</button>
                                    </div>
                                )}
                            </div>
                                ...
                                <div className="info-right">
                                    {layout === 'list' && (
                                        <button 
                                            ...
                                            onClick={() => handleQuickView(p, salePrice)}
